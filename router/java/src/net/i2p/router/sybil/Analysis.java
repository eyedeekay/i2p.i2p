...
****/
    ...
    public List<RouterInfo> getFloodfills(Hash us) {
        ...
        for (Hash ff : ffs) {
             if (ff.equals(us))
                 continue;
             RouterInfo ri = _context.netDb(null).lookupRouterInfoLocally(ff);
             if (ri != null)
                 ris.add(ri);
        }
        return ris;
    ...
    public List<RouterInfo> getAllRouters(Hash us) {
        Set<RouterInfo> set = _context.netDb(null).getRouters();
    ...
    public synchronized Map<Hash, Points> backgroundAnalysis(boolean includeAll) {
        ...
        for (Hash client : destinations) {
            ...
            if (! _context.clientManager().shouldPublishLeaseSet(client))
                continue;
            LeaseSet ls = _context.netDb(null).lookupLeaseSetLocally(client);
            if (ls == null)
                continue;

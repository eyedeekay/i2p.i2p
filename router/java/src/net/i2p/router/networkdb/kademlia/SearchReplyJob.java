...
class SearchReplyJob extends JobImpl {
    ...
    private void processPeer(int curIndex) {
            ...
            RouterInfo info = getContext().netDb(null).lookupRouterInfoLocally(peer);
            if (info == null) {
                ...
                if (!sendsBadInfo) {
                    ...
                    //    getContext().statManager().addRateData("netDb.searchReplyValidationSkipped", 1, 0);
                        ...
                        //getContext().netDb(null).lookupRouterInfo(peer, new ReplyVerifiedJob(getContext(), peer), new ReplyNotVerifiedJob(getContext(), peer), _timeoutMs);

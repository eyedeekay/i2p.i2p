...
public abstract class TunnelPeerSelector extends ConnectChecker {
    ...
    private boolean shouldExclude(Hash h, boolean isInbound, boolean isExploratory) {
        ...
        RouterInfo info = (RouterInfo) ctx.floodfillNetDb().lookupLocallyWithoutValidation(h);
        if (info == null)
            return true;
    ...
    protected boolean allowAsOBEP(Hash h) {
        RouterInfo ri = (RouterInfo) ctx.floodfillNetDb().lookupLocallyWithoutValidation(h);
        if (ri == null)
            return true;
    ...
    private class ClosestHopExcluder extends ExcluderBase {
        ...
        public boolean contains(Object o) {
            ...
            if (ctx.commSystem().isEstablished(h))
                return false;
            boolean canConnect;
            RouterInfo peer = (RouterInfo) ctx.floodfillNetDb().lookupLocallyWithoutValidation(h);
            if (peer == null) {
                canConnect = false;
            } else if (isIn) {

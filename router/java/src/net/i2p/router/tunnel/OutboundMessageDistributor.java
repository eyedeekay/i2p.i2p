...
class OutboundMessageDistributor {
    ...
    public void distribute(I2NPMessage msg, Hash target, TunnelId tunnel) {
        ...
        RouterInfo info = _context.netDb(null).lookupRouterInfoLocally(target);
        if (info == null) {
            ...
            _context.netDb(null).lookupRouterInfo(target, new DistributeJob(_context, msg, target, tunnel), null, MAX_DISTRIBUTE_TIME);
            return;
        } else {
    ...
    private class DistributeJob extends JobImpl {
        ...
        public void runJob() {
            RouterInfo info = getContext().netDb(null).lookupRouterInfoLocally(_target);
            int stat;
            if (info != null) {
                if (_log.shouldLog(Log.DEBUG))

...
class OutboundReceiver implements TunnelGateway.Receiver {
    ...
    public OutboundReceiver(RouterContext ctx, PooledTunnelCreatorConfig cfg) {
        ...
        _config = cfg;
        _nextHopCache = _context.netDb(null).lookupRouterInfoLocally(_config.getPeer(1));
    ...
    public long receiveEncrypted(byte encrypted[]) {
        ...
        if (ri == null) {
            ri = _context.netDb(null).lookupRouterInfoLocally(_config.getPeer(1));
            _nextHopCache = ri;
        }
        if (ri != null) {
            send(msg, ri);
            ...
            if (_log.shouldLog(Log.WARN))
                _log.warn("lookup of " + _config.getPeer(1)
                           + " required for " + msg);
            _context.netDb(null).lookupRouterInfo(_config.getPeer(1), new SendJob(_context, msg),
    ...
    private class SendJob extends JobImpl {
        ...
        public void runJob() {
            RouterInfo ri = _context.netDb(null).lookupRouterInfoLocally(_config.getPeer(1));
            if (_log.shouldLog(Log.DEBUG))

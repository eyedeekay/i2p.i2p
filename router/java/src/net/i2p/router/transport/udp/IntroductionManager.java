...
class IntroductionManager {
    ...
    public int pickInbound(RouterAddress current, boolean ipv6, Properties ssuOptions, int howMany) {
        ...
        for (int i = 0; i < sz && found < howMany; i++) {
            ...
            RouterInfo ri = _context.netDb(null).lookupRouterInfoLocally(hash);
            if (ri == null) {
    ...
    void receiveRelayRequest(PeerState2 alice, byte[] data) {
        ...
        if (charlie == null) {
            ...
            aliceRI = _context.netDb(null).lookupRouterInfoLocally(alice.getRemotePeer());
            if (aliceRI != null) {
    ...
    private boolean receiveRelayIntro(PeerState2 bob, Hash alice, byte[] data, int retryCount) {
        ...
        if (retryCount >= 5) {
            // last chance
            aliceRI = _context.netDb(null).lookupRouterInfoLocally(alice);
    ...
    void receiveRelayResponse(PeerState2 peer, int status, byte[] data) {
        ...
        if (alice != null) {
            ...
            if (true) {
                RouterInfo charlie = _context.netDb(null).lookupRouterInfoLocally(peer.getRemotePeer());
                if (charlie != null) {
                    byte[] signedData;
                    if (status == 0)

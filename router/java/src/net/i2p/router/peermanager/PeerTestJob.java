...
class PeerTestJob extends JobImpl {
    ...
    private Set<RouterInfo> selectPeersToTest() {
        ...
        for (Hash peer : peerHashes) {
            RouterInfo peerInfo = getContext().netDb(null).lookupRouterInfoLocally(peer);
            if (peerInfo != null) {
                peers.add(peerInfo);
            } else {
    ...
    private void testPeer(RouterInfo peer) {
        ...
        RouterInfo inGateway = getContext().netDb(null).lookupRouterInfoLocally(inTunnel.getPeer(0));
        if (inGateway == null) {
            if (_log.shouldLog(Log.WARN))
